...
namespace CleanArchWithCQRSandMediatr.Application.Blogs.Queries.GetBlogs
{
    public class GetBlogQueryHandler : IRequestHandler<GetBlogQuery, List<BlogVm>>
    {
        ...
        {
            var blogs = await _blogRepository.GetAllBlogsAsync(); // ret domain entity
            // convert domain entity into vm entity using LINQ lambda
            //var blogList =  blogs.Select(x => new BlogVm 
            //  { Author = x.Author, Name = x.Name, 
            //      Description = x.Description , Id = x.Id }).ToList();
            // convert domain entity into vm entity using IMapper
